...
instacart
```
The size of the dataset `Instacart` is `r nrow(instacart)` observtaions and `r ncol(instacart)` variables. Key variables include all associated details of the order and the product acquired. The mean hour of day where orders were made is `r mean(pull(instacart, order_hour_of_day))`. THe median number of days between orders is `r median(pull(instacart, days_since_prior_order))`
...
instacart_one
```
There are 134 aisles. Aisle 83, 24, 123 ane 120 are where the most items were ordered from. 
